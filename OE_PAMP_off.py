...
#   OE_PAMP function to offload microservice istance-sets from cloud cluster to edge cluster


def OE_PAMP_off(RTT, AVG_DELAY, APP, APP_EDGE, RCPU, RMEM, Rs, M, SLO, lambda_value, CTX_CLUSTER2, NAMESPACE, prom, SLO_MARGIN_UNOFFLOAD, PERIOD, MICROSERVICE_DIRECTORY, HPA_DIRECTORY, NE):
    ...
    best_S_edge = np.array(output[0]) # Istance-sets that must stay in the edge cluster according to OE_PAMP
    ...
    new_edge_names = np.array(np.array(APP))[new_edge == 1] # Microsrvice istance-sets to offload to the edge cluster
