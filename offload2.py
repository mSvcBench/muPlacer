...
def offload(Rcpu, Rmem, Fcm, M, lambd, Rs, app_edge, delta_mes, RTT, Ne):
    ...
    Scurr_cloud_b = Sold_b[0:M] # Binary placement status containing only edge microservices
    ...
    while True:
        ...
        for path_id in dependency_paths_cloud_only_r_id :
            ...
            Fci_temp = np.matrix(buildFci(S_temp_b, Fcm, M, e))
        ...
        if Sopt_id == Snew_edge_id:
            ...
            for pr,dependency_path_cloud_only_r_id in enumerate(dependency_paths_cloud_only_r_id):
                if np.bitwise_and(dependency_path_cloud_only_r_id - 1, Snew_edge_id - 1) + 1 == dependency_path_cloud_only_r_id:
