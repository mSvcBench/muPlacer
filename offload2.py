...
def offload(Rcpu_old, Rmem_old, Fcm, M, lambd, Rs, S_edge_old, delta_mes, RTT, Ne,depth):
    ...
    Cost_edge_old = Cost_cpu_edge_old_sum + Cost_mem_edge_old_sum # Total cost
    ...
    if delta_mes > 0:
        ...
        for ms in range(M-1):
            ...
            for path_n in paths_n:
                edge_presences = S_b_old[M:][path_n]
    ...
    ## UNOFFLOAD ##
        ...
        S_b_old=np.ones(2*M)
        S_b_old[M-1:2*M-1] = 0
        Sold_edge_b = S_b_old[M:2*M] # For unoffload, the old status has no edge microservice
    ...
    Rcpu_opt = Rcpu_old.copy()
    Rmem_opt = Rmem_old.copy()
    ...
    while True:
        ...
        print(f'new ms {np.argwhere(S_b_new[M:]==1).squeeze()}, delta_delay {1000*(delay_old-delay_new)}, cost {Cost_edge_new}, delta_cost/delta_delay {delta_cost_opt/(1000*delta_opt)}') if debug else 0
        
        # Check if the delay reduction is enough
        if delay_old-delay_new >= delta_target:
        ...
        for i in r :
            ...
            # weighting
            r_delta = delta_target - (delay_old-delay_new) # residul delay to decrease wrt previous conf
            if delta_delay < 0:
    ...
    while True:
        ...
        for i in range(M, 2*M-1):
            if S_b_new[i]==1:
                ...
                Fci_temp = np.matrix(buildFci(S_b_temp, Fcm, M))
                Nci_temp = computeNc(Fci_temp, M, 2)
                delta_final2 = delay_old - computeDnTot(S_b_temp, Nci_temp, Fci_temp, Rs, RTT, Ne, lambd, M)
