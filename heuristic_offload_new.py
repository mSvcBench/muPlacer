...
def heuristic_offload(Fcm, RTT, Rcpu_req, Rcpu, Rmem, Cost_cpu_edge, Cost_mem_edge, Ce, Me, Ne, lambd, Rs, M, db, horizon, e, Sold_b, delta_mes):
    ...
    Sold_edge_b = Sold_b[M:2*M-1] # Binary placement status containing edge microservices only
    ...
    # Sorigin[:M - 1] = 1
    # Sorigin[M:] = S_old_edge_b

    # delay_old = delayMat(Sorigin, Fcm, Rcpu, Rcpu_req, RTT, Ne, lambd, Rs, M, 2) # Delay of the original configuration
    # dependency_paths_cloud_only_r_id = dependency_paths_cloud_only_r_id[1:] # Remove the configuration with all instances in the edge cluster
