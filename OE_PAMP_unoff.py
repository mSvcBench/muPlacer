...
#   OE_PAMP function to unoffload microservice istance-sets from edge cluster
...
def OE_PAMP_unoff(RTT, AVG_DELAY, APP, APP_EDGE, RCPU, RMEM, Rs, M, SLO, lambda_value, CTX_CLUSTER2, NAMESPACE, prom, SLO_MARGIN_UNOFFLOAD, PERIOD, MICROSERVICE_DIRECTORY, HPA_DIRECTORY, NE):
    ...
    if not np.array_equal(best_S_edge, APP_EDGE):
        new_edge = np.subtract(APP_EDGE,best_S_edge) # Microservice istance-sets to delete from edge cluster
        to_delete = np.array(np.array(APP))[new_edge == 1] # Name of microservice istance-sets to delete from edge cluster
    else:
