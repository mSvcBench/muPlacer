...
def Kahn_heuristic(params):
    ...
    if not kahn_topo_order:
        ...
        if len(kahn_topo_order) == n:
            return kahn_topo_order
        else:
    ...
    if delay_decrease_target > 0:
        ...
        while delay_decrease_target > delay_decrease_new:
            ...
            edge_vals = S_b_new[M + kahn_topo_order]  # Get the edge values of the microservices in topo_order
            idxs = np.where(edge_vals > 0)[0]
            first_idx = idxs[0] if idxs.size > 0 else None
            candidate_ms = kahn_topo_order[first_idx:]
            S_b_new[kahn_topo_order[candidate_ms]+M] = 1
