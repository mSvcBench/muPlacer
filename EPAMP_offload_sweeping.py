...
def offload(params):
    ...
    while True:
        ...
        for cgw in cloud_gws:
            ...
            delay_temp_gw,_,_,_ = computeDTot(S_b_temp, Nci_temp, Fci_temp, Di, Rs, RTT, Ne, lambd, M, np.empty(0))
            utils.computeResourceShift(Acpu_temp,Amem_temp,Nci_temp,Acpu_old,Amem_old,Nci_old)
            cost_gw = utils.computeCost(Acpu_temp[M:], Amem_temp[M:], Qcpu[M:], Qmem[M:], Cost_cpu_edge, Cost_mem_edge)[0] # Total edge cost of the temp state

            # children weighthing for sweeping
            wi = 1e6*np.ones(len(cloud_gw_children))
            for chi,ch in enumerate(cloud_gw_children):
                S_b_temp[M+ch] = 1
                Fci_temp = np.matrix(buildFci(S_b_temp, Fcm, M))
                Nci_temp = computeNc(Fci_temp, M, 2)
                delay_temp_ch,_,_,_ = computeDTot(S_b_temp, Nci_temp, Fci_temp, Di, Rs, RTT, Ne, lambd, M, np.empty(0))
                utils.computeResourceShift(Acpu_temp,Amem_temp,Nci_temp,Acpu_old,Amem_old,Nci_old)
                cost_ch = utils.computeCost(Acpu_temp[M:], Amem_temp[M:], Qcpu[M:], Qmem[M:], Cost_cpu_edge, Cost_mem_edge)[0] # Total edge cost of the temp state
                if delay_temp_ch > delay_temp_gw:
                    # penalty for adding dependency path that increase the delay
                    wi[chi] = 1e6 - (cost_ch - cost_gw) * 1000 * (delay_temp_gw-delay_temp_ch)
                else:
                    wi[chi] = (cost_ch - cost_gw)/(1000*(delay_temp_gw-delay_temp_ch))
                S_b_temp[M+ch] = 0
            
            ci = np.argsort(wi)
            for j in range(len(ci)):
                path_b_single_ch = path_b_gw.copy()
                path_b_single_ch[0,cloud_gw_children[ci[j]]] = 1 # single children dependency path
                dependency_paths_b = np.append(dependency_paths_b,path_b_single_ch,axis=0)
                if j>0 and no_sweeping==False:
                    path_b_sweep_ch = path_b_gw.copy()
                    path_b_sweep_ch[0,cloud_gw_children[ci[:j+1]]] = 1  # sweeping dependency path
                    dependency_paths_b = np.append(dependency_paths_b,path_b_sweep_ch,axis=0)
