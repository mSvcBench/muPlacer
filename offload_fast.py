...
def offload_fast(params):
    ...
    S_b_old = np.concatenate((np.ones(int(M)), S_b_edge_old)) # (2*M,) Initial status of the instance-set in the edge and cloud. (:M) binary presence at the cloud, (M:) binary presence at the edge
    ...
    Fci_opt = Fci_old.copy()  # Fci_opt is the best instance-set call frequency matrix computed by a greedy round 
    ...
    for n in range(0,N):
        ...
        for L in range(1,len(values) + 1):
            for subset in itertools.combinations(values, L):
                ...
                k=k+1
                # combs[n] = combs[n].append(list(subset)) 
    ...
    while True:
        ...
        # identify gateway microservice
        S_n_edge_new = np.argwhere(S_b_new[M:]==1)[:,0] # index of edge microservices
        edge_gateway_indexes = np.argwhere(np.sum(Fci_opt[S_n_edge_new+M, :M].squeeze(), axis=1)>0)[:,0] # index in S_n_edge_new of edge microservices with outgoing calls towards cloud
        edge_gateways = S_n_edge_new[edge_gateway_indexes] # index of edge microservices with outgoing calls towards cloud
        edge_gateway_peer_group = dict() # list of groups of cloud microservices called by the same gateway microservices
        edge_gateway_peer_group_combs = dict() # list of all possible combination of cloud microservices called by the same gateway microservices
        dependency_paths_b_residual = np.empty((0,M), int) # Storage of binary-based (b) encoded dependency paths
        for eg in edge_gateways:
            edge_gateway_peer_group[eg] = np.argwhere(Fci_opt[eg+M, :M].squeeze()>0)[:,1]
            C = combs[len(edge_gateway_peer_group[eg])-1] 
            edge_gateway_peer_group_combs[eg] = []
            for c in C:
                path_b = np.zeros((1,M),int)
