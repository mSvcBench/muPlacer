...
def offload_ns(params):
    ...
    Rs = params['Rs']
    Di = params['Di']
    delay_increase_target = params['delay_increase_target'] if 'delay_increase_target' in params else 0
    delay_decrease_target = params['delay_decrease_target'] if 'delay_decrease_target' in params else 0
    RTT = params['RTT']
    Ne = params['Ne']
    ...
    delay_target = delay_old + delay_increase_target - delay_decrease_target
    ...
    logging.info(f"offload_ns calls offload with void edge state with delay_decrease_target: {delay_decrease_target} and estimated void delay: {delay_void}")
    result = offload(params)
    result['delay_increase'] = (delay_void-result['delay_decrease']) - delay_old
    result['delay_decrease'] = - result['delay_increase']
    result['cost_decrease'] = Cost_edge_old-result['Cost']
    result['cost_increase'] = - result['cost_decrease']
    return result
