...
def offload(params):
    ...
    def evaluate_perf(S_b_new, Acpu_old, Amem_old, Nci_old, round):
        ...
        if not hit:
            ...
            Fci_new = np.matrix(buildFi(S_b_new, global_Fcm, global_M))
            Nci_new = computeN(Fci_new, global_M, 2)
    ...
    global_Fci_old = np.matrix(buildFi(global_S_b_old, global_Fcm, global_M)) # (2*M,2*M) instance-set call frequency matrix
    global_Nci_old = computeN(global_Fci_old, global_M, 2)  # (2*M,) number of instance call per user request
    ...
    while True:
        ...
        for leaf in edge_leaves:
            ...
            Fci_temp = np.matrix(buildFi(S_b_temp, global_Fcm, global_M))
            Nci_temp = computeN(Fci_temp, global_M, 2)
