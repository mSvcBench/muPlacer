...
def offload(params):
    ...
    look_ahead = 1.3 # look ahead factor to increase the delay decrease target
    ...
    round = -1
    added_dp = np.empty((0,M),dtype=int) # list of added dependency paths
    ...
    while True:
        ...
        for dpi,path_b in enumerate(dependency_paths_b_residual[rl]) :
            ...
            if rhoce == 1 or delay_decrease_temp <= 0:
                ...
                Fci_all_next_in = buildFci(S_all_next_in, Fcm, M)
                Nci_all_next_in = computeNc(Fci_all_next_in, M, 2)
...
#                w = cost_increase_temp/dependency_paths_b_w[dpi] /  max(min(1000*delay_decrease_temp, 1000*r_delay_decrease),1e-3) # 1e-3 used to avoid division by zero
        ...
        added_dp = np.append(added_dp,dp_best,axis=0)
    ...
    dependency_paths_b_added = added_dp
    ...
    while True:
        ...
        dp_with_edges = np.argwhere(dependency_paths_b_added[:,edge_leaves].sum(axis=1)>0)[:,0]
        
        #logger.info(f'pruning for leaves {edge_leaves}')
        np.copyto(S_b_temp,S_b_new)
        delay_reduction_v = np.zeros((len(dp_with_edges),2))   # delay reduction and weigth for each possible removal
        for dpi,path_b in enumerate(dependency_paths_b_added[dp_with_edges]):
            dependency_paths_b_temp = np.delete(dependency_paths_b_added,dp_with_edges[dpi],axis=0)
            
            S_b_temp[M:] = np.minimum(np.sum(dependency_paths_b_temp,axis=0),1)
            S_b_temp[M:] = S_b_temp[M:]+S_b_old[M:]
            ...
            delay_reduction_v[dpi] = [delay_old - delay_temp, w]
        
        feasible_dpi = np.argwhere(delay_reduction_v[:,0]>=delay_decrease_target).flatten()
        if len(feasible_dpi)>0:
            dpi_best = np.argmax(delay_reduction_v[feasible_dpi][:,1])
            dpi_best = feasible_dpi[dpi_best]
        else:
            feasible_dpi = np.argwhere(delay_reduction_v[:,0]>delay_decrease_target/look_ahead).flatten()
            if len(feasible_dpi)>0:
                dpi_best = np.argmax(delay_reduction_v[feasible_dpi][:,1])
                dpi_best = feasible_dpi[dpi_best]
        
        if dpi_best>-1:
            delay_reduction_best = delay_reduction_v[dpi_best,0]
            logger.info(f'cleaning dependency path {np.argwhere(dependency_paths_b_added[dp_with_edges[dpi_best]]).squeeze()}, delay reduction: {delay_reduction_best}')
            dependency_paths_b_added = np.delete(dependency_paths_b_added,dp_with_edges[dpi_best],axis=0)
            S_b_new[M:] = np.minimum(np.sum(dependency_paths_b_added,axis=0),1)
            S_b_new[M:] = S_b_new[M:]+S_b_old[M:]
            S_b_new[S_b_new>0] = 1
