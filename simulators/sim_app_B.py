...
B_v = np.arange(0.2e9,2e9,0.2e9)  # bitrate cloud-edge
#B =1e9
...
for k in range(trials):
    ...
    for B in B_v:
        ...
        print(f'\n bandwidth {B/1e9} Gbit/s')
        delay_decrease_target = max(delay_void - target_delay,0)
        Ti+=1   # index of the result vector   
...
savemat(f"res1_B_pareto{barabasi['pareto_shape']}.mat", mdic)
